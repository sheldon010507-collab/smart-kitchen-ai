...
const getApiKey = () => {
  ...
  if (meta && meta.env) {
    ...
    if (meta.env.GEMINI_API_KEY) return meta.env.GEMINI_API_KEY;
  }
  return '';
};
...
export const generateThinkingChefRecipes = async (inventory: InventoryItem[], menuItems: MenuItem[] = []): Promise<Recipe[]> => {
  ...
  const prompt = `
    ...
    Rules:
    1. **Prioritize using ingredients that are expiring soon** (top of the inventory list) to reduce waste.
    ...
    4. **Highlight Waste Reduction**: In the description of the recipe, explicitly mention which "soon-to-expire" ingredients are being utilized (e.g., "Uses likely-to-expire Milk").
...
export const generateOperationalInsights = async (
  ...
  try {
    if (!apiKey) {
      console.error("API Key is missing for geminiService");
    ...
    console.error("Failed to generate operational insights:", e);
    if (e.message?.includes("429") || e.message?.includes("quota")) {
      return "Error: API Quota exceeded. Please try again later.";
    }
    return `Failed to generate insights: ${e.message || "Unknown error"}`;
  }
};
