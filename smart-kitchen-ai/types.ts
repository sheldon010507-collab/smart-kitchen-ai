...
export interface Business {
  ...
  setupStatus?: 'onboarding' | 'active'; // Onboarding state for Setup Wizard
}

// ============================================
// Category & Location Management (Setup Wizard)
// ============================================
...
// ============================================
// Inventory Setup Wizard Types
// ============================================

export type IngestionPath = 'import' | 'template' | 'photo';

export type SetupStage = 1 | 2 | 3 | 4;

export interface ColumnMapping {
  sourceColumn: string;
  targetField: 'name' | 'category' | 'unit' | 'cost' | 'supplier' | 'location' | 'minStockLevel' | null;
...
export interface DraftInventoryItem {
  ...
  errors: string[];      // e.g., ['Missing unit']
  warnings: string[];    // e.g., ['Par level is 0']
}
...
export interface IndustryTemplate {
  ...
  items: { name: string; category: string; unit: string; suggestedPar?: number }[];
  locations: { name: string; type: 'Dry' | 'Fridge' | 'Freezer' }[];
}
...
export interface InventoryItem {
  ...
  location: string;
  locationId?: string; // FK to locations table
  expiryDate: string;
  addedDate: string;
  minStockLevel?: number; // Standard stock level (maps to min_stock_level in DB)
}
